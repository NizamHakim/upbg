...
<x-app-layout>
  <x-slot name="title">Kategori Tes</x-slot>
  <x-slot name="header">Kategori Tes</x-slot>
  ...
  <section>
    <div class="flex flex-col">
      ...
      <div class="kategori-container paginated-table">
        @include('tes.partials.kategori-table', ['kategoriList' => $kategoriList])
      </div>
    </div>
  </section>

  @pushOnce('modals')
    <x-modal id="tambah-kategori-modal" route="{{ route('kategori-tes.store') }}">
  ...
  @pushOnce('scripts')
    <script src="{{ asset('js/tes/kategori/daftar-kategori.js') }}"></script>
  @endPushOnce
</x-app-layout>
