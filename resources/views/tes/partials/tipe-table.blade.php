      ...
      @foreach ($tipeList as $tipe)
        <div class="table-item grid-cols-12">
          ...
          <div class="tipe-item contents">
            @include('tes.partials.tipe-item', ['tipe' => $tipe, 'deleted' => $deleted])
          </div>
        </div>
      @endforeach
    @endif
...
<div class="flex flex-col">
  {{ $tipeList->links() }}
  @if (!$deleted)
    <a href="{{ route('tipe-tes.deleted') }}" class="mt-2 w-fit self-center underline decoration-gray-600 transition hover:text-upbg-light hover:decoration-upbg-light sm:mt-0 sm:self-start">restore</a>
  @endif
</div>
