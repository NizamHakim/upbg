...
<x-app-layout>
  <x-slot name="title">Tipe Tes</x-slot>
  <x-slot name="header">Tipe Tes</x-slot>
  ...
  <section>
    <div class="flex flex-col">
      ...
      <div class="tipe-container paginated-table">
        @include('tes.partials.tipe-table', ['tipeList' => $tipeList])
      </div>
    </div>
  </section>

  @pushOnce('modals')
    <x-modal id="tambah-tipe-modal" route="{{ route('tipe-tes.store') }}">
    ...
    <x-modal id="hapus-tipe-modal">
      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="alert alert-red flex flex-col gap-2">
          ...
          <p>Hapus permanen akan menghapus tipe dari database dan semua data tes yang terasosiasi dengan tipe ini!</p>
        </div>
  ...
  @pushOnce('scripts')
    <script src="{{ asset('js/tes/tipe/daftar-tipe.js') }}"></script>
  @endPushOnce
</x-app-layout>
